...
required_cols <- c("participant_id", "additive", "flavors", "gender", "covid_status", "Beer")
...
long_df$Beer[is.na(long_df$Beer)] <- "Unknown"
...
permanova_res <- vegan::adonis2(
  dist_bc ~ additive + gender + covid_status + Beer,
  data = rows,
  permutations = 999,
...
if (length(target_flavs) > 0) {
  capture_warnings({
    for (flav in target_flavs) {
      ...
      m <- try(
        suppressWarnings(
          glmer(y ~ additive + gender + covid_status + Beer + (1 | participant_id),
      ...
      if (inherits(m, "try-error")) {
        ...
        m <- try(suppressWarnings(glm(y ~ additive + gender + covid_status + Beer,
